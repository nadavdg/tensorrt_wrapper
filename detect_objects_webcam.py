...
MODEL_NAME = '/home/tevel/Downloads/yolo4_32_engine.trt'
...
def show_inference_results(img, result, threshold=0.5, width=640, height=480):
    ...
    for bbox in result:
        ...
        img = cv2.rectangle(img, (int(bbox[0] * width), int(bbox[1] * height)),
                            (int(bbox[2] * width), int(bbox[3] * height)), (0, 255, 0), 2)
...
def main():
    ...
    trt_inference_wrapper = inference_utils.TRTInference(
        "/home/tevel/Downloads/yolo4_32_engine.trt", None,
    ...
    if args.camera == True:
        ...
        frame = cv2.imread("/home/tevel/Desktop/vid_221114_075832_C.avi_0016.png")
        ...
        while True:
            ...
            detection_out, scores = trt_inference_wrapper.infer_webcam(image_np)
            bbox = detection_out.reshape(-1,4)

            ind_th = np.where(scores > 0.005)[0]
            indexes = nms_python(bbox[ind_th], scores[ind_th], 0.9)
            bbox = bbox[ind_th[indexes]]
            show_inference_results(frame, bbox, 0.8)

            # Overlay the bounding boxes on the image
            # let analyze_prediction() draw them based on model output
            img_pil = Image.fromarray(image_np)
            prediction_fields = len(TRT_PREDICTION_LAYOUT)
            for det in range(len(keep_count_out)):
